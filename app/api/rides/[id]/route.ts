...
async function getCurrentVolunteerID(): Promise<number | null> {
  try {
    ...
    return volunteerInfo?.id || null;
  } catch (error) {
    console.error('Error getting current volunteer ID:', error);
    return null;
  }
}
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        // If a ride is being reserved and no volunteerID is provided, get the current user's volunteer ID
        if (isReserving && !updateData.volunteerID) {
            const currentVolunteerID = await getCurrentVolunteerID();
            if (currentVolunteerID) {
                updateData.volunteerID = currentVolunteerID;
                console.log(`[RideUpdate] Auto-assigning volunteer ID ${currentVolunteerID} to ride ${id}`);
            } else {
                console.warn(`[RideUpdate] Could not determine volunteer ID for reservation of ride ${id}`);
            }
        }
        ...
        if (Object.keys(prismaUpdateData).length > 0) {
            ...
            const finalFormattedDataForNoUpdate = rideWithUpdatedData ? {
                ...
                volunteerName: rideWithUpdatedData.volunteer?.user ? `${rideWithUpdatedData.volunteer.user.firstName} ${rideWithUpdatedData.volunteer.user.lastName}` : ''
            } : null;
