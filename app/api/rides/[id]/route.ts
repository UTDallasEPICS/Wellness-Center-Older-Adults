...
            };
            ...
            if (isReserving && ADMIN_EMAIL && updatedRide.customer && updatedRide.volunteer) {
                
                const NEWLY_ASSIGNED_VOLUNTEER_EMAIL = updatedRide.volunteer.user?.email;
                const VOLUNTEER_NAME = `${updatedRide.volunteer.user?.firstName} ${updatedRide.volunteer.user?.lastName}` || 'A Volunteer';
                const dateString = updatedRide.date ? new Date(updatedRide.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : 'TBD';
                const timeString = finalFormattedData.startTime || 'TBD';
                        ...
                        html: `
                            <div style="font-family: Arial, sans-serif; line-height: 1.6; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #10b981;">
                                ...
                                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin-bottom: 20px;">
                                    ...
                                    <tr><td style="padding: 5px 0;"><strong style="color: #4a5568;">Volunteer:</strong> ${VOLUNTEER_NAME}</td></tr>
                                    <tr><td style="padding: 5px 0;"><strong style="color: #4a5568;">Volunteer Email:</strong> ${NEWLY_ASSIGNED_VOLUNTEER_EMAIL || 'N/A'}</td></tr>
                                </table>
                                <p style="font-size: 14px; color: #718096;">Please log into the dashboard for full details.</p>
                            </div>
                        `,
                    };
                    try {
