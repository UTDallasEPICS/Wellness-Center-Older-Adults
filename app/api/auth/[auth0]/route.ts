    ...
    if (auth0 === 'callback') {
        try {
            ...
            if (!id_token) {
                ...
                return NextResponse.redirect('/?error=missing_id_token', { status: 302 });
            }
            ...
            try {
                ...
                console.log('Decoded JWT Payload:', decoded.payload); // Log the payload for debugging
            } catch (error) {
            ...
            if (!email) {
                ...
                return NextResponse.redirect('/?error=email_not_found', { status: 302 });
            }
            ...
            if (!user) {
                console.log(`User with email ${email} not found in the database. Redirecting to logout.`);
            ...
            const redirectResponse = NextResponse.redirect(`http://localhost:3000${redirectTo}`, { status: 302 });
            ...
            console.log(`User ${user.id} logged in. Redirecting to: ${redirectTo}`);
