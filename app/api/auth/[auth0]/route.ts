...
export async function POST(request: Request, { params }: { params: { auth0: string } }) {
    ...
    if (auth0 === 'callback') {
        try {
            ...
            let redirectTo = `/Dashboard/volunteers`; // Default redirect for non-admins (volunteers)

            // Check if the user is an admin (assuming you have an 'isAdmin' field in your User model)
            if (user.isAdmin) {
                redirectTo = '/Dashboard/dashboardEmployee/rides/page'; // Redirect for admins
            }

            const ret = NextResponse.redirect(`http://localhost:3000${redirectTo}`, { status: 302 });
            ret.cookies.set({
                name: 'cvtoken',
                value: `${id_token}`,
                httpOnly: true,
            ...
            ret.cookies.set('cvuser', `${user.id}`);
            return ret;
        } catch (error) {
