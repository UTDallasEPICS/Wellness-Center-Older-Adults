  ...
  const handleUnreserveRide = async () => {
    if (rideDetails) {
      try {
        ...
        router.push("/Dashboard/rides-volunteer?tab=available");
      } catch (err) {
        console.error("Error updating ride status:", err);
    ...
    if (rideDetails) {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            `Failed to update ride: ${response.status} - ${errorData?.error || "Unknown error"}`
          );
        }
    ...
    <div className="flex w-full h-full bg-white font-sans text-gray-800">
        ...
        <div className="flex-grow">
          ...
          <div className="flex justify-between mb-6 border-b border-gray-100 pb-4">
            ...
            <p className="text-gray-600 font-normal text-xl mt-2">
              Date: {formatDate(rideDetails.date)}
            </p>
          </div>
          ...
          {!isEditing ? (
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-3 pl-4 border-l-4 border-green-600 text-sm">
                  <label className="block">
                    ...
                    <input
                      ...
                      className="w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring focus:ring-green-200 focus:ring-opacity-50 transition-colors p-2 text-gray-700"
                    />
                  </label>
                  <label className="block">
                  ...
                  <label className="block">
                    ...
                    <input
                      type="text"
                      name="finalAddress"
                      value={rideDetails.finalAddress || ""}
                      disabled
                      className="w-full mt-1 border-gray-200 bg-gray-50 rounded-md shadow-sm p-2 text-gray-500 cursor-not-allowed"
                    />
                  </label>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-x-12 gap-y-4 pt-4 border-t border-gray-100">
                <label className="col-span-1">
                  ...
                  <input
                    ...
                    value={pickupTime}
                    onChange={handleInputChange}
                    className="w-full border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring focus:ring-green-200 focus:ring-opacity-50 transition-colors p-2 text-gray-700"
                  />
                </label>
                <div className="flex flex-col">
              ...
              <div className="flex gap-4 pt-4 border-t border-gray-200">
                ...
                <button
                  className="flex-grow px-5 py-3 bg-gray-200 text-gray-700 font-semibold rounded-md shadow-lg hover:bg-gray-300 transition-colors"
        ...
        <div className="pt-4 mt-auto">
          ...
          {!isEditing && userRole === "VOLUNTEER" && (
            <div className="flex gap-4">
              ...
              rideDetails.status === "Added" ||
              rideDetails.status === "Unreserved" ? (
                <>
                  <button
                    className="flex-grow px-5 py-3 bg-green-600 text-white font-semibold rounded-md shadow-md hover:bg-green-700 transition-colors"
                ...
                <>
                  <button
                    className="flex-grow px-5 py-3 bg-gray-500 text-white font-semibold rounded-md shadow-md hover:bg-gray-300 transition-colors"
      ...
      <div className="w-1/2 h-full">
        {rideDetails?.pickupAddress && rideDetails?.dropoffAddress && (
          <RideMap
            pickupAddress={
              isEditing ? pickupAddress : rideDetails.pickupAddress
            }
            dropoffAddress={
              isEditing ? dropoffAddress : rideDetails.dropoffAddress
            }
